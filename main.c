...
/*   Updated: 2023/05/22 14:17:39 by hece             ###   ########.tr       */
/*                                                                            */
/* ************************************************************************** */
...
//-------------------
	objects->camera = camera_set(aspect_ratio, 66, vector3_set(0, -0.5, -1), point3_set(0, 10, 20));

	objects->sphere = add_sphere(objects->sphere, point3_set(-2, 2.5, -1), 0.5, color3_set(1,0,0));
	//objects->sphere = add_sphere(objects->sphere, point3_set(0, -100.5, -1), 100, color3_set(0.8, 0.8, 0.0));
	//objects->sphere = add_sphere(objects->sphere, point3_set(-1, 1, -2), 0.5, color3_set(0.7, 0.3, 0.3));
	//objects->sphere = add_sphere(objects->sphere, point3_set(1, -0.6, -2), 0.5, color3_set(0.5, 0.5, 0.5));
	//objects->sphere = add_sphere(objects->sphere, point3_set(4, 5, -5), 1, color3_set(0.5, 0.5, 0.5));

	objects->plane = add_plane(objects->plane, point3_set(1,0,-10), vector3_set(0, 0, 1), color3_set(0.1, 1, 0));
	objects->plane = add_plane(objects->plane, point3_set(0,0,0), vector3_set(0, 1, 0), color3_set(1,0, 1));
	
	objects->cylinder = add_cylinder(objects->cylinder, point3_set(3, 1, -1.6),
							cylinder_params(5, 21, vector3_set(0, 1, 0)), color3_set(0, 0, 1));
	//objects->cylinder = add_cylinder(objects->cylinder, point3_set(2, 1, -5),
	//						cylinder_params(1, 5, vector3_set(-0.7, 0.2, 1)), color3_set(1, 0.2, 0));
	//objects->cylinder = add_cylinder(objects->cylinder, point3_set(2, 2, -5),
	//						cylinder_params(1, 5, vector3_set(-0.7, 0.2, -1)), color3_set(0, 0.5, 1));

	objects->point_light = set_point_light(color3_set(1, 1, 1), 0.7f, point3_set(-4, 2, 5));
	objects->ambient_light = set_ambient_light(color3_set(0.3, 0.5, 1.0), 0.5f);
}*/

static bool check_vars(int argc)
{
	if (argc == 1)
		write(2, "Incomplete number of arguments.\n", 32);
	else if (argc > 2)
